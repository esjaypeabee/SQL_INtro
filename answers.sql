...
-- Problem 24
SELECT common_name, melon_type, quantity, unit_price, total_price FROM melons JOIN order_items ON (order_items.melon_id = melons.id) WHERE order_items.order_id = 2725;

-- Problem 25
select SUM(order_total) FROM orders WHERE salesperson_id IS NULL;

-- Problem 26
SELECT givenname, surname, SUM(orders.order_total), (SUM(orders.order_total)*.15) FROM salespeople LEFT JOIN orders ON (salespeople.id = orders.salesperson_id) GROUP BY orders.salesperson_id
